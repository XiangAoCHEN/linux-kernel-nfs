...
extern const char *LDB_file_filter_list[LDB_MAX_FILE_NUM];
//struct idr LDB_fd2filename;
static inline char *my_get_file_path(struct file *file, char *buf, int buflen) {
	char *path = NULL;
	path = d_path(&file->f_path, buf, buflen);
	// d_path returns a pointer within the buffer or an error code
	if (IS_ERR(path))
		return NULL;
	return path;
}
static int my_in_filter_list(const char *file_path) {
	size_t path_len, suffix_len;
	const char *suffix_pos;

	path_len = strlen(file_path);
	for (size_t i = 0; i < LDB_MAX_FILE_NUM; i++) {
		if (LDB_file_filter_list[i] == NULL) // End of list
			break;

		suffix_len = strlen(LDB_file_filter_list[i]);
		if (path_len < suffix_len) // If the file path is shorter than the suffix, continue.
			continue;

		// Find the position where the suffix would start to match the end of the file path
		suffix_pos = file_path + (path_len - suffix_len);

		if (strcmp(suffix_pos, LDB_file_filter_list[i]) == 0)
			return 1; // Suffix match found
	}
